...
interface ButtonAsAnchorProps extends BaseButtonProps, React.AnchorHTMLAttributes<HTMLAnchorElement> {
  as: "a";
  href?: string;
  target?: string;
}
...
>((props, ref) => {
  ...
  const computedClassName = `${baseStyle} ${variants[variant]} ${sizes[size]} ${className || ''}`;

  if (as === "a") {
    const { href, target, ...anchorProps } = rest as ButtonAsAnchorProps;
    return (
      <a
